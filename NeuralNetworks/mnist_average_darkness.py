...
shown an image the classifier returns whichever digit in the
training data had the closest average darkness.
...
digits! Still, it's useful to show what sort of performance we get from
naive ideas.
...
def guess_digit(image, avgs):
    ...
    closest to the darkness of ``image``. Note that ``avgs`` is assumed
    to be a defaultdict whose keys are 0...9, and whose values are the
